      ...
      $dbhost = 'localhost:3036';
		...
		if($conn)
		{
			...
			if (isset($_POST["guardar"])) 
			{
				...
				$query = "update styles set descripcion = '$descripcion' where id = '$fila[id]'";
						...
						if(isset($_POST["borrar"])) 
							{
								...
								$fila = $_POST["borrar"];
								$q = "delete from styles where id = '$fila[id]'";
								mysql_select_db('breakTime');
								$res = mysql_query($q);
	...
	while($fila = mysql_fetch_array($retval))
	{
		...
		if ($edito)
		 {
			...
			// Listado con edición
			 echo "<form action='formListarStyles.php' method='post'>";
			 echo "<tr>";		
			 ...
			 <a href='formListarStyles.php' class='btn btn-danger'>Cancelar</a>
			 </td>";
			 echo "</tr>";
			 echo "</form>";		 
		...
		// Listado con borrado
		...
		else
		 {
		     ...
		     echo "<td class='text-nowrap'>";
			 echo '<a href="formListarStyles.php?editar='.$fila['id'].'" class="btn btn-primary">Editar</a>';
			 echo '<a href="formListarStyles.php?borrar='.$fila['id'].'" class="btn btn-danger">Borrar</a>';
			 echo "</td>";
			 echo "</tr>";
				$i++;
		 		$count++;
